...
| Fix Akochan GitHub Actions workflow validation blocker | 04/07/09 | P0 | First workflow run failed validation | Workflow no longer uses unsupported job-level `runner.temp` context and can be rerun manually | Done locally: pending commit/push and rerun |
| Run Akochan F1 GitHub Actions build audit | 04/07/09 | P0 | Corrected manual workflow pushed | Workflow run logs show whether Ubuntu build produces `system.exe` and at least one minimal non-training sample succeeds | Planned: current next |
