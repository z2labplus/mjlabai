...
| P4 | Unified mahjong environment and interface | Define shared state, legal actions, logs, replays and adapter contracts | Different candidates can run through the same interface | Active prerequisite: synthetic transition is review-closed in `04I`; pinned MahJax `v0.1.2` init/observe/legal-step integration is review-closed in `04K`; bounded single-round approval/deferment is next; no full conformance or shared candidate adapter yet |
