...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | `04AJ` accepts the exact alternate `116..147` four-pass sensitivity implementation: parameters change but rewards retain `-312/-1056`, so reference improvement is seed-protocol sensitive and no protocol is selected; the approved fast no-training robustness evidence gate is implemented and awaiting lightweight review, while further protocols/windows, fifth-pass tuning, robust improvement and strength claims remain unapproved |
