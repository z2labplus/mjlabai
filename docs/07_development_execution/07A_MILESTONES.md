...
| P6 | Data system | Build replay, feature, label and quality pipelines for training and evaluation | Supervised training and offline evaluation datasets can be generated | Docs-only replay schema and fixture implementation readiness checklist reviewed; implementation not approved |
...
The next execution step is to define the P6 replay schema and synthetic fixture
implementation proposal boundary before code. That task must remain docs-only
and must not implement fixture files, replay schema code, data ingestion,
