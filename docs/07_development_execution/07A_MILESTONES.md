...
| P4 | Unified mahjong environment and interface | Define shared state, legal actions, logs, replays and adapter contracts | Different candidates can run through the same interface | Single-round MahJax path is established; `04AU` directly approves the first exact pinned local half-game rule-policy rollout as current P8 environment prerequisite; full candidate-general conformance remains open |
...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | `04AU` closes the four-pass negative-result review, stops that mechanism branch and directly approves one exact pinned local MahJax half-game rule-policy environment smoke; no project-model half-game training, strength or P9-P12 yet |
