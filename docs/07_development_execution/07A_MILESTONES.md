...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | The `04AK` two-protocol leave-one-out batch-baseline diagnostic is implemented: both exact branches change parameters but retain initial `-312/-1056` fixed-window rewards, so no protocol is selected and lightweight review is current; third protocols/windows, fifth-pass tuning, robust improvement and strength claims remain unapproved |
