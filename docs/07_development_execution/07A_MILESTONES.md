...
| P6 | Data system | Build replay, feature, label and quality pipelines for training and evaluation | Supervised training and offline evaluation datasets can be generated | Docs-only minimal replay schema and synthetic fixture implementation proposal reviewed with no blocker; implementation not approved |
...
The next execution step is to prepare an approval decision for the minimal P6
replay schema and synthetic fixture implementation task. That task must remain
a docs-only approval-decision gate and must not implement fixture files, tests,
replay schema code, data ingestion, dataset readers, parsers, feature
extraction, label generation, training, self-play, league, real Tenhou,
