...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | MahJax bridge through estimator/rate failures, census and seed-protocol comparison is executable; `04AD` accepts the comparison and directly approves ordered `0..31` training plus disjoint `52..83` evaluation after probe sums `-312/-312` and changed seeds `(52,65,72)`; no checkpoint selection, production evaluation or strength claim is approved |
