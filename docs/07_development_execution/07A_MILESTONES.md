...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | `04AO` review-closes the one-step projection and directly approves one exact four-pass shared-policy continuation with the same fixed method and final-only windows. Nothing is selected; search, robust improvement and strength claims remain unapproved |
