...
| P4 | Unified mahjong environment and interface | Define shared state, legal actions, logs, replays and adapter contracts | Different candidates can run through the same interface | Exact pinned MahJax half-game rule-policy path now reaches natural termination in 938 legal transitions with complete round provenance; lightweight implementation review is current; full candidate-general conformance remains open |
...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | Four-pass unit-gradient branch is stopped; exact half-game rule-policy environment smoke is implemented and awaits lightweight review before any read-only project-model half-game diagnostic; no strength or P9-P12 yet |
