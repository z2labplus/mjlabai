...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | `04AG` review-accepts the MahJax exact four-pass/128-attempt causal-baseline diagnostic; process-local immutable raw-reference reuse is implemented and reduces the full 498-test run to 5454.853 seconds; one exact reuse review is next, while checkpoint selection, robust improvement and strength claims remain unapproved |
