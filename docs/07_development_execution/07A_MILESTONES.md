...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | `04AG` review-accepts the MahJax exact four-pass/128-attempt causal-baseline diagnostic; disjoint evaluation changes `-312` to `-297` and negatives `20` to `19` with positives unchanged at `2`; direct process-local raw-reference test reuse is next to reduce the realized 5890.810-second regression runtime, while checkpoint selection, robust improvement and strength claims remain unapproved |
