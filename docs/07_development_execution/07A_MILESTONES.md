...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | Fixed linear model training is accepted in `12AQ`; exact synthetic/local inference and greedy-decision diagnostic is directly approved and next; no environment, self-play or production model training approved |
