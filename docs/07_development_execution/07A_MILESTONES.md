...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | The exact `04AN` one-step projection is implemented: cosine flips positive and parameters change, but fixed rewards remain `-312/-1056`. Nothing is selected and lightweight review is current; search, robust improvement and strength claims remain unapproved |
