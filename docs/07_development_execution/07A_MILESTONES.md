...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | `04AK` accepts the fast no-selection robustness gate after alternate `116..147` fails to reproduce reference fixed-window gains; it directly approves one real two-protocol four-pass leave-one-out batch-baseline variance-control diagnostic, while third protocols/windows, fifth-pass tuning, selection, robust improvement and strength claims remain unapproved |
