...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | MahJax bridge through estimator/rate failures and frozen-policy census is executable; the exact selected `(1,3,5,7,11)` versus contiguous `(0,1,2,3,4)` comparison is implemented and pins `-501/-650/-501` with no selected protocol; one exact review is next, with no production evaluation or strength claim approved |
