...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | MahJax bridge, imitation training, held-out use and mixed-policy round are implemented/reviewed; first exact seed-1 on-policy raw-outcome gradient update is directly approved next; no production self-play, evaluation or model-strength claim approved |
