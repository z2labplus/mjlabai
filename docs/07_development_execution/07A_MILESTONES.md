...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | `04AM` review-closes the fixed `32x` diagnostic: reference `+54/+121` is not reproduced by alternate `-60/0`, so no branch is selected. One exact no-update first-pass aggregate-gradient alignment diagnostic is directly approved to explain protocol sensitivity; scale/seed search, robust improvement and strength claims remain unapproved |
