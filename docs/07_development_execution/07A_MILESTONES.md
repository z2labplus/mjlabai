...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | MahJax bridge through estimator/rate failures, census and seed-protocol comparison is executable; ordered `0..31` raw versus causal-running-baseline comparison is implemented with 10/32 versus 31/32 nonzero updates and identical disjoint reward sum `-312`; baseline changes one complete raw evaluation record without reward improvement; one exact review is next, and no estimator/checkpoint selection or strength claim is approved |
