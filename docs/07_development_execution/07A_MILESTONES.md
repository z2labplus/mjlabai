...
| P7 | Supervised policy model | Train a base strategy model from high-quality human play and key decisions | Model beats simple baselines in key offline scenarios and completes games | Data/source readiness inventory defined/reviewed; feature/label boundary defined/reviewed; risk/evidence taxonomy next; implementation unapproved |
...
reviews that boundary and records `Review can close`. The current next step is
a docs-only P7 supervised-learning risk and evidence taxonomy definition
