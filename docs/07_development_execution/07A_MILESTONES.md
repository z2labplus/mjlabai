...
| P4 | Unified mahjong environment and interface | Define shared state, legal actions, logs, replays and adapter contracts | Different candidates can run through the same interface | Active prerequisite: synthetic transition, pinned integration and bounded rollout are review-closed; exact bundled rule-policy round is implemented with 325 tests passing and exact review next; no full conformance/shared project-model adapter yet |
