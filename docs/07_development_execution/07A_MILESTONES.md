...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | MahJax raw-versus-causal-baseline comparison is review-accepted; a predeclared exact four-pass/128-attempt probe changes disjoint evaluation from `-312` to `-297` and negative rounds `20` to `19` with positives unchanged at `2`; its exact bounded implementation is directly approved next, while checkpoint selection, robust improvement and strength claims remain unapproved |
