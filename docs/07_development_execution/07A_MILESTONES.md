...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | Exact `04AP` per-trajectory influence diagnostic is implemented: opposite alignment is mixed `14-/18+` vs `18-/14+` across all 64 trajectories with zero updates/evaluations. Lightweight review is current; nothing is selected and strength remains unapproved |
