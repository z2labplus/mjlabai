...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | First fixed synthetic/local linear action-value model training smoke is implementation-review closed in `12AP`; exact inference/greedy-decision acceptance/approval decision is next; no environment, self-play or production model training approved |
