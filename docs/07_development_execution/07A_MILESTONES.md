...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | The `04AM` no-update first-pass alignment diagnostic is implemented: exact two-batch aggregate-gradient dot `-0.0001429308562` and cosine `-0.1868768328` expose conflicting training signals. Nothing is selected and lightweight review is current; search, robust improvement and strength claims remain unapproved |
