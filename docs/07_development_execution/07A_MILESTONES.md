...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | Fixed table smoke is accepted in `12AK`; exact fixed two-pass policy-table update sequence is directly approved and next; no environment, self-play or production RL training approved |
