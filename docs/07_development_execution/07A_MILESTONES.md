...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | MahJax bridge, categorical MLP training/first four-seat update and seeds-(1,3) sequential updates are implemented/reviewed; exact five-update training plus independent fixed evaluation is implemented and pins regression `-320->-454`; one exact review is next, with no production self-play/evaluation or strength claim approved |
