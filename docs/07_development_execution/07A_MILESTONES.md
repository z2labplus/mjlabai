...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | MahJax bridge through five-update/fixed-evaluation and estimator/rate failures is executable; the exact frozen-policy `0..31` census is implemented and confirms 10/32 nonzero outcomes versus 5/5 in the reference training tuple; one exact review is next, with no replacement split, algorithm selection, production evaluation or strength claim approved |
