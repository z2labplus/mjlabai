...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | Exact minimal synthetic/local single-record policy-update smoke implemented in the three `12AC`-approved files and awaiting exact review; no environment, self-play or production RL training approved |
