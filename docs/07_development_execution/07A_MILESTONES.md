...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | MahJax bridge, first imitation gradient smoke and held-out trained-policy round are implemented/reviewed; exact one-project-seat/three-rule-seat mixed-policy round is directly approved next; no reward/RL update, production self-play or model-strength claim approved |
