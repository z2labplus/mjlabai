...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | Fixed two-key policy-table smoke is implemented and review-closed in `12AJ` with 90 tests passing; current-scope acceptance and direct next executable-task decision is next; no environment, self-play or production RL training approved |
