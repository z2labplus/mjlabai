...
| P8 | Self-play reinforcement learning | Optimize toward Tenhou pt EV, placement and stable-dan objectives | RL checkpoint beats supervised checkpoint in the unified league | Exact trace is accepted in `12AI`; fixed two-key in-memory policy-value table update smoke is directly approved and next; no environment, self-play or production RL training approved |
