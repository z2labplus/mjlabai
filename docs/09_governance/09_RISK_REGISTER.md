...
| GitHub-visible Mahjong Soul samples are mistaken for licensed training data | Compliance / Data | Critical | Realized | The bounded smoke preserves `rights_status=unverified` and finds the inspected sample is not training-ready; prohibit platform access, bulk acquisition, committed raw logs and training until permitted use is recorded | Mitigated for feasibility smoke |
| Parsed Mahjong Soul analysis transcript is mistaken for raw replay training data | Data / Semantics | Critical | Realized | Classify content before conversion; the fixed sample is explicitly `majsoul_analysis_transcript`, has potential identifiers and cannot enter labels/datasets | Mitigated for fixed sample |
...
| Risk | Category | Severity | Probability | Mitigation | Status |
|---|---|---|---|---|---|
| Bounded trainer silently becomes unbounded production training. | Governance / Scope | High | Medium | Exact tuple input, hard `1..8` validation and focused lower/upper-bound tests are implemented; no while loop, schedule, retry or early stopping exists. | Mitigated |
| Loop changes reviewed update semantics or loses table continuity. | Engineering / Correctness | High | Medium | Implementation calls the reviewed table helper once per ordered pass and feeds each final table directly to the next; focused tests verify both keys. | Mitigated |
| Duplicate records across variable passes hide replay/reuse. | Engineering / Correctness | Medium | Medium | Implementation flattens validated IDs and rejects duplicates across all passes; focused coverage confirms the failure. | Mitigated |
...
| Finite but non-float-representable numeric input leaks a raw built-in exception. | Engineering / API | Medium | Low | Conversion overflow is normalized, regression-tested and confirmed by `12AD` re-review. | Closed |
...
| Risk | Category | Severity | Probability | Mitigation | Status |
|---|---|---|---|---|---|
| `12R` review closure is mistaken for reward selection, P8 entry or implementation approval. | Governance / Stage Control | High | Medium | `12R`, handoff, stage contract and `10_NEXT` classify it as review evidence only and select another docs-only boundary task. | Open |
| Evaluation-only metrics are silently reused as reward components. | Research / Evidence | High | Medium | `12R` records that the family is classification/evaluation only and requires separate review before any reuse. | Open |
| Generic provenance fields fail to bind future reward records to protocol, environment or raw-outcome versions. | Engineering / Reproducibility | High | Medium | Future schema review must add explicit immutable upstream identities; this review grants no schema approval. | Open |
| Retry lineage or failed episodes are omitted, creating success-only selection bias. | Evaluation / Integrity | High | Medium | Future records must preserve episode/retry lineage, seeds, eligibility and invalid/aborted denominators. | Open |
| The next environment/simulator boundary task is mistaken for environment implementation or execution approval. | Governance / Execution | High | Medium | `10_NEXT` is docs-only and forbids environments, simulators, runners, transitions, episodes, self-play, RL and training execution. | Open |
| Objective/reward review evidence is overclaimed as strength, Tenhou, stable-dan, LuckyJ or promotion evidence. | Evidence / Governance | High | High | Evidence grade remains objective/reward boundary review evidence only. | Open |
...
| Risk | Category | Severity | Probability | Mitigation | Status |
|---|---|---|---|---|---|
| Dataset / split / leakage boundary review closure is mistaken for supervised dataset construction approval. | Governance / Scope | High | Medium | `03AH`, `10_NEXT`, handoff and stage contract state that `Review can close` closes only the review gate and approves no dataset construction. | Open |
| The next training-data approval / training-run boundary task is mistaken for actual training approval. | Governance / Training | High | Medium | The new first `10_NEXT` item is docs-only and forbids training-data approval, training-run approval, training, model architecture, trainer, checkpoints and weights. | Open |
| Source, parser / reader / ingestion, feature or label approvals are assumed from dataset boundary review. | Data / Governance | High | Medium | `03AH` preserves the dependency map and reiterates that these approvals remain separate and non-substitutable. | Open |
| Feature tensors, labels, targets, examples, dataset records or manifests are generated before approval. | Data / Training | High | Medium | `03AH` and `10_NEXT` explicitly forbid tensors, labels, targets, examples, records, manifests, splits and training data. | Open |
| Split creation or leakage checks start before exact future tasks are approved. | Data / Leakage | High | Medium | `03AH` confirms that `03AG` only defines split/leakage boundaries and requires separate future approval for exact files and checks. | Open |
| Real Tenhou, real haifu, external logs or platform data are used as dataset or training inputs before approval. | Data / Compliance | High | Medium | `03AH` keeps real/external/platform data blocked until source-specific and implementation approvals exist. | Open |
| Boundary review evidence is overclaimed as model-strength, Tenhou ranked, stable-dan or LuckyJ evidence. | Evaluation / Governance | High | Medium | Evidence grade is broader P7 supervised dataset construction, split and leakage boundary review evidence only. | Open |
| P8-P12 work is treated as implicitly approved after dataset / split / leakage boundary review. | Governance / Stage Control | High | Medium | `03AH`, stage contract and `10_NEXT` keep P8-P12 closed and require later transition reviews. | Open |
...
| Risk | Category | Severity | Probability | Mitigation | Status |
|---|---|---|---|---|---|
| Handoff/evidence finalization is mistaken for P7 current-scope closure. | Governance / Scope | High | Medium | `03U`, `10_NEXT`, handoff and stage contract state that P7 current scope remains open until the final closure review gate. | Open |
| Evidence index is mistaken for broader P7 implementation approval. | Governance / Scope | High | Medium | `03U` marks all implementation/training/source/parser/extraction/label/model/P8-P12 approvals as `no`. | Open |
| Final closure review skips validation or governance consistency checks. | Governance / Process | Medium-High | Medium | `03U` records final closure readiness and requires the existing P7/P6 validation commands to be rerun. | Open |
| Handoff evidence is overclaimed as model-strength, Tenhou ranked, stable-dan or LuckyJ evidence. | Evaluation / Governance | High | Medium | `03U` evidence grade is handoff/evidence finalization evidence only with explicit non-evidence warnings. | Open |
| P8-P12 work is treated as implicitly approved by handoff finalization. | Governance / Stage Control | High | Medium | `03U` and `10_NEXT` keep P8-P12 unapproved and select only the final P7 current-scope closure review gate next. | Open |
