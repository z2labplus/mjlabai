...
| Finite but non-float-representable numeric input leaks a raw built-in exception. | Engineering / API | Medium | Low | Conversion overflow is now normalized and covered by an exact regression test; `12AD` re-review remains. | Mitigated pending re-review |
