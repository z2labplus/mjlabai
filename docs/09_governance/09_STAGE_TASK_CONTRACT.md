...
with all 413 explicit tests passing. Exact seeds `(1,3)` produce legal 77/84-
step nonzero rounds; two actor-indexed `0.01` updates carry shared arrays
directly and change all four final parameter groups. Zero planning gates remain
before code; another proposal/boundary is forbidden.
...
Implement the exact `04W` categorical-MLP two-round sequential all-project-seat
training smoke. No proposal, boundary or review may intervene before code.
