...
The exact implementation is complete and passes 136 tests. The current first
task is its one code review; no sibling proposal or boundary may precede it.
Production code
...
Review the exact `12AQ`-approved synthetic/local linear-model inference and
greedy-decision implementation. Verify fixed inputs, provenance, helper reuse,
values, tie behavior, frozen output, errors, exports, tests and warnings
without changing code unless a concrete blocker exists. Broad P8, source/
real-data work,
