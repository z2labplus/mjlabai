...
| Project model-output environment bridge | Encode only public decision-time observation and mask 87 project-owned linear model scores by environment legality | Exact 630-feature, 54,897-parameter untrained linear-policy round review-closed in `04O`; seed 0 terminates after 91 legal transitions |
| Environment-backed imitation training smoke | Update project parameters from local bundled-rule-policy public decisions without persisted data | Exact seed-0/seed-1 54/64-example, 16-epoch full-batch training approved in `04O`; direct implementation next |
