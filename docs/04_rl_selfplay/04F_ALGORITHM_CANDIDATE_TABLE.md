...
| Akochan | Open-source baseline / reviewer candidate | C++ Japanese Mahjong AI with self-match, JSON/mjai/log review and legal-action related components. | Public repo; Ubuntu GitHub Actions F1 run generated `system.exe` and passed minimal `legal_action` plus `mjai_log` samples. F2 real-exe runs pass `legal_action`; cwd blocker is mitigated; real `mjai_log` stdout parser blocker remains. | Medium | Medium-High for Ubuntu build/minimal-run; Low for local macOS build | Fix the real `mjai_log` stdout parser/diagnostics and rerun the F2 workflow without uploading third-party artifacts. | F2 parser blocker; real `mjai_log` not yet compatible. |
...
2. Fix the minimal Akochan F2 wrapper parser for real external Ubuntu-built `mjai_log` stdout, then rerun fixed `legal_action` / `mjai_log` samples under the documented guardrails.
...
| Akochan | F2 parser blocker | 修复真实 `mjai_log` stdout 解析/诊断后重跑真实 GitHub Actions Ubuntu-built external `system.exe` workflow，不上传或保存第三方 artifact。 | F1 Conditional Pass 已有 Ubuntu GitHub Actions run `26617347785` 证据；最小 wrapper skeleton 已通过 fake-executable smoke tests；run `26621536548` 暴露并推动修复 cwd blocker；run `26623247276` 证明真实 `legal_action` 通过且 `setup_mjai.json` blocker 消失，但真实 `mjai_log` stdout 解析失败。 |
