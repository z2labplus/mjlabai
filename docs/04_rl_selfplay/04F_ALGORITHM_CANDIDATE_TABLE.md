...
| Akochan | Open-source baseline / reviewer candidate | C++ Japanese Mahjong AI with self-match, JSON/mjai/log review and legal-action related components. | Public repo; Ubuntu GitHub Actions F1 run generated `system.exe` and passed minimal `legal_action` plus `mjai_log` samples. F2 first real-exe run passed `legal_action` but found an `mjai_log` cwd blocker; local cwd fix is implemented. | Medium | Medium-High for Ubuntu build/minimal-run; Low for local macOS build | Rerun the F2 wrapper workflow against a real GitHub Actions Ubuntu-built external `system.exe` with `AKOCHAN_WORKING_DIR` set, without uploading third-party artifacts. | F2 wrapper cwd fix implemented; real external workflow rerun next. |
...
2. Rerun the minimal Akochan F2 wrapper against a real external Ubuntu-built `system.exe` with `AKOCHAN_WORKING_DIR` set for fixed `legal_action` / `mjai_log` samples under the documented guardrails.
...
| Akochan | F2 wrapper cwd fix implemented | 重跑真实 GitHub Actions Ubuntu-built external `system.exe` workflow，确认 `AKOCHAN_WORKING_DIR` 下固定 `legal_action` / `mjai_log` 样例是否都通过，不上传或保存第三方 artifact。 | F1 Conditional Pass 已有 Ubuntu GitHub Actions run `26617347785` 证据；最小 wrapper skeleton 已通过 fake-executable smoke tests；run `26621536548` 证明真实 `legal_action` 通过且暴露 `mjai_log` cwd blocker；cwd 修复已本地实现，仍需 workflow 复验。 |
