...
| Akochan | Open-source baseline / reviewer candidate | C++ Japanese Mahjong AI with self-match, JSON/mjai/log review and legal-action related components. | Public repo; Ubuntu GitHub Actions F1 run generated `system.exe` and passed minimal `legal_action` plus `mjai_log` samples. F2 fixed-sample real-exe wrapper validation passed in run `26629344590`: fake tests passed 14 tests, real `legal_action` passed and real `mjai_log` passed. This is not strength evidence. | Medium | Medium-High for Ubuntu build/fixed-sample wrapper validation; Low for local macOS build | Keep as interface/reviewer candidate. Any broader evaluator/reviewer integration must be a separate task with license boundary review. | F1 Conditional Pass; F2 fixed-sample wrapper validation passed; not a strength baseline. |
...
2. Build the evaluation metric foundation, starting with the Tenhou stable-dan calculator from room-specific formulas.
...
| Akochan | F1 Conditional Pass + F2 fixed-sample wrapper validation passed | 暂停扩展 Akochan adapter；如后续要做 evaluator/reviewer 接入，必须另起任务并复查 custom license 边界。 | F1 Conditional Pass 已有 Ubuntu GitHub Actions run `26617347785` 证据；run `26629344590` 证明 fixed-sample real-exe wrapper validation 通过，fake tests 14 tests passed，real `legal_action` 和 real `mjai_log` 均通过。该证据不是强度证据。 |
