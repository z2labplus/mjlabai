...
| Akochan | Open-source baseline / reviewer candidate | C++ Japanese Mahjong AI with self-match, JSON/mjai/log review and legal-action related components. | Public repo; Ubuntu GitHub Actions F1 run generated `system.exe` and passed minimal `legal_action` plus `mjai_log` samples. | Medium | Medium-High for Ubuntu build/minimal-run; Low for local macOS build | Run the minimal F2 wrapper skeleton against a real GitHub Actions Ubuntu-built external `system.exe` for fixed `legal_action` / `mjai_log` samples without uploading third-party artifacts. | F2 wrapper skeleton implemented; real external validation next. |
...
2. Validate the minimal Akochan F2 wrapper skeleton against a real external Ubuntu-built `system.exe` for fixed `legal_action` / `mjai_log` samples under the documented guardrails.
...
| Akochan | F2 wrapper skeleton implemented | 用真实 GitHub Actions Ubuntu-built external `system.exe` 验证固定 `legal_action` / `mjai_log` 样例，不上传或保存第三方 artifact。 | F1 Conditional Pass 已有 Ubuntu GitHub Actions run `26617347785` 证据；`07J_AKOCHAN_F2_INTERFACE_TASK.md` 已定义边界；最小 wrapper skeleton 已通过 fake-executable smoke tests，但真实 `system.exe` 兼容性仍需验证。 |
