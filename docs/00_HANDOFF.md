...
definition planning after minimal synthetic/local feature-label smoke
acceptance and next-task definition:
...
```text
Define P7 current-scope closure criteria after minimal synthetic feature-label
smoke acceptance.
The closure-criteria definition must be docs-only. It must not add production
