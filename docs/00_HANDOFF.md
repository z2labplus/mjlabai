...
Current active stage is P7 supervised learning current-scope closure criteria
review planning after minimal synthetic/local feature-label smoke acceptance:
...
current scope; the next task is a docs-only closure-criteria review gate.
...
```text
Review P7 current-scope closure criteria after minimal synthetic feature-label
smoke acceptance.
The review gate must be docs-only. It must not add production code, tests,
