...
当前 `10_NEXT` 的下一步是用真实外部 `system.exe` 验证最小 Akochan F2 wrapper skeleton：
...
下一步允许的范围只包括在临时 GitHub Actions Ubuntu runner 或其他明确批准的外部路径中，用真实构建出的 Akochan `system.exe` 跑固定 `legal_action` / `mjai_log` 样例；仍然不训练、不调参、不自我对弈、不接入 Tenhou、不 vendor 或上传第三方源码、二进制、`params/` 或未知 artifact。
