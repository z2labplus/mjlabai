...
```text
P6 data-system minimal implementation approval.
P5 evaluation foundation is closed for the current synthetic/local scope.
General P6 data-system implementation is not open; only the next exact
minimal replay schema and project-authored synthetic fixture task is approved.
...
P6 implementation = approved only for the next exact minimal replay schema and project-authored synthetic fixture task.
Next = implement minimal P6 replay schema and project-authored synthetic fixture only.
```
...
approves only the next exact minimal implementation task. The current next
task may implement only the minimal replay schema module, project-authored
synthetic/local fixture and two minimal tests named in `02N`; all parser,
dataset reader, ingestion, feature extraction, label generation, real-data,
CLI, model-output, training, self-play, league and P7-P12 work remains
unapproved.
