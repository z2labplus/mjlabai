...
and ingestion boundary before implementation` as the next docs-only task. None
of `03Y`, `03Z`, `03AA`, `03AB` or the next parser / reader / ingestion
boundary task approves broader P7 implementation, training, source approval,
source ingestion, parser / reader / ingestion implementation, actual feature
...
review can close with no blocker.
Broader P7 parser / reader / ingestion boundary = current docs-only next task
before implementation.
P7 broad implementation = not approved.
P8-P12 entry = not approved.
P6 implementation = closed except for separately approved future tasks.
Next = define broader P7 parser, reader and ingestion boundary before
implementation.
```
...
docs-only parser / reader / ingestion boundary definition next. The next task
must not add production code, tests, fixtures, data files, source approval,
source ingestion approval, data reads, parser implementation, dataset reader
