...
当前 `10_NEXT` 的下一步是重跑真实外部 `system.exe` wrapper workflow，验证 strict JSON stream parser 是否解除 `mjai_log` stdout blocker：
...
本地 wrapper cwd 边界已修复：`AkochanWrapper` 支持显式 `working_dir`、`AKOCHAN_WORKING_DIR` 和默认 `Path(system_exe).resolve().parent`；`subprocess.run` 使用 `cwd=self.working_dir`；audit log 记录 `working_dir`。workflow run `26623247276` 证明 `setup_mjai.json` blocker 已消失，真实 `legal_action` 继续通过，但真实 `mjai_log` stdout 触发 `JSONDecodeError: Extra data`。本地 parser 已支持 single JSON、JSON Lines、concatenated JSON values 和 pretty-printed multi-record JSON stream，并禁止 partial parse 当成功。下一步只允许重跑该 GitHub Actions workflow 并复审日志；仍然不训练、不调参、不自我对弈、不接入 Tenhou、不 vendor 或上传第三方源码、二进制、`params/` 或未知 artifact。
